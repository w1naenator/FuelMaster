...
public class VehicleRepositoryImpl implements VehicleRepository {
   ...
   @Autowired
    ...
    public Page<Vehicle> findAll(Pageable pageable) {
        ...
        List<Vehicle> tests = query.getResultList();
	...
	public Vehicle findByNumber(String number) {
        ...
        Predicate predicate = builder.equal(root.get("name"), number);

        query.where(predicate);

        List<Vehicle> fuels = sessionFactory.getCurrentSession().createQuery(query).getResultList();
        if (fuels.isEmpty())return null;
        else return fuels.get(0);
